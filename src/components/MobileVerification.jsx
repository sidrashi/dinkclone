...
import { OtpAPI, resendOtpAPI } from "../api/Api";
...
function MobileVerification() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await OtpAPI(userId, finalOtp);
      showToast(res.data);
      if (res.data.error === 0) {
        navigate("/login");
      }
  ...
  return (
    <div className="max-w-[425px] py-12 mx-auto px-4">
      <div className="flex justify-start items-center w-full font-custom mb-6">
        <RoundedButton
          ...
          icon={<IoMdArrowRoundBack className="ml-2 w-9 h-5 md:w-12 md:h-7" />}
        />
      ...
      <div className="text-center font-ptag text-darkbrown text-lg leading-5 font-normal mb-4">
        <p>Please enter your 6 digit OTP number here to verify your account.</p>
      </div>
        ...
        <div className="w-full flex justify-between items-center">
          {otp.map((value, index) => (
            <input
              ...
              className="size-12 text-center text-xl bg-white border rounded-xl focus:outline-none"
            />
          ))}
        </div>
